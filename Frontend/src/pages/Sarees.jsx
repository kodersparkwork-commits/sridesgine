    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.products && data.products.length > 0) {
          ...
          setProducts(dummyProducts.filter(p => p.category === 'sarees'));
        }
      } else {
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="bg-secondary/30 py-16 px-4">
        <div className="max-w-7xl mx-auto text-center">
          <h1 className="font-serif text-4xl md:text-5xl text-primary mb-4">Sarees Collection</h1>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex gap-8">
            ...
            <main className="flex-1 min-w-0">
              ...
              {loading ? (
                <div className="text-center py-20 text-text-muted">Loading sarees...</div>
              ) : error ? (
                <div className="text-center py-20 text-red-600">{error}</div>
              ) : filteredProducts.length === 0 ? (
                <div className="text-center py-20">
                  <p className="text-text-muted text-lg">No sarees match your current filters. Try adjusting your search criteria.</p>
                </div>
              ) : (
                <>
